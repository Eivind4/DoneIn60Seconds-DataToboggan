...
string[] calcTypes = new string[] {
    "Actual", "MTD", "QTD", "YTD", "WTD", "R12", "Rolling average", "Rolling total", "Start and End"
};
...
if (selectedValues.Contains("WTD"))
{
    var _selectedTableName = _CalendarDate.Split('[')[0].Trim('\'');
    var _selectedDateTable = Model.Tables[_selectedTableName];
    var _calendarColumns = _selectedDateTable.Columns
        .Where(c => c.DataType == DataType.Int64 || c.DataType == DataType.Double) // Only numeric columns
        .ToList();

    try
    {
        // Select the ISO-year column
        var isoYearCol = SelectColumnFromList(_calendarColumns, "WEEK TO DATE CALCULATIONS:\nSelect the ISO_YEAR-COLUMN to be used\n(assumption: created in calendar-table)");
        if (isoYearCol == null)
        {
            Error("No ISO Year column selected. WTD calculations will be skipped.");
        }
        else
        {
            _ISO_year = isoYearCol.DaxObjectFullName;
            
            // Select the ISO-week column
            var isoWeekCol = SelectColumnFromList(_calendarColumns, "WEEK TO DATE CALCULATIONS:\nSelect the ISO_Week-COLUMN (WW-format) to be used\n(assumption: created in calendar-table)");
            if (isoWeekCol == null)
            {
                Error("No ISO Week column selected. WTD calculations will be skipped.");
                _ISO_year = null; // Reset since we need both
            }
            else
            {
                _ISO_week = isoWeekCol.DaxObjectFullName;
                
                // Select the day number column
                var dayNoCol = SelectColumnFromList(_calendarColumns, "WEEK TO DATE CALCULATIONS:\nSelect the DAY NUMBER COLUMN to be used\n(assumption: created in calendar-table)");
                if (dayNoCol == null)
                {
                    Error("No Day Number column selected. WTD calculations will be skipped.");
                    _ISO_year = null; // Reset since we need all
                    _ISO_week = null;
                }
                else
                {
                    _dayNoOfWeek = dayNoCol.DaxObjectFullName;
                    
                    // NEW: Select the ISO Year Week Running Number column
                    var isoYearWeekRunningCol = SelectColumnFromList(_calendarColumns, "WEEK TO DATE CALCULATIONS:\nSelect the ISO YEAR WEEK RUNNING NUMBER COLUMN to be used\n(assumption: created in calendar-table)");
                    if (isoYearWeekRunningCol == null)
                    {
                        Error("No ISO Year Week Running Number column selected. WTD calculations will be skipped.");
                        _ISO_year = null; // Reset since we need all four
                        _ISO_week = null;
                        _dayNoOfWeek = null;
                    }
                    else
                    {
                        _ISO_yearWeekRunningNumber = isoYearWeekRunningCol.DaxObjectFullName;
                    }
                }
            }
        }
    ...
    {
        Error("Error selecting WTD columns. WTD calculations will be skipped.");
        _ISO_year = null;
        _ISO_week = null;
        _dayNoOfWeek = null;
        _ISO_yearWeekRunningNumber = null;
    }
}
...
RETURN
 ...
 if (item == "WTD")
    {
        if (!string.IsNullOrEmpty(_ISO_year) && !string.IsNullOrEmpty(_ISO_week) && !string.IsNullOrEmpty(_dayNoOfWeek) && !string.IsNullOrEmpty(_ISO_yearWeekRunningNumber))
        {
            AddCalculationItemIfNotExists(
                "WTD",
                string.Format(@"// https://www.sqlbi.com/articles/week-based-time-intelligence-in-dax
                // NB! 1.Shows the entire week without hiding future dates
                //     2. A more optimized DAX code can be found: https://www.daxpatterns.com/week-related-calculations/
                //        This pattern is valid for general concept, but new pattern is suggested (ref. SQLBI article reference)
IF (
    HASONEVALUE ({0})
        && HASONEVALUE ({1} ),
    CALCULATE (
    ...
    BLANK ()
)", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate),
                ordinal++,
                "Accumulated week to date for ISO week",
                null
            );
            ...
            AddCalculationItemIfNotExists(
                "WTD LY",
                string.Format(@"// https://www.sqlbi.com/articles/week-based-time-intelligence-in-dax
// Shows the entire week without hiding future dates
...
IF (
    ...
    BLANK ()
)", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek),
                ordinal++,
                "Accumulated week to date same ISO week last year",
                null
            );
...
VAR _Result = _thisYear-_lastYear
RETURN _Result", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek),
                ordinal++,
                "WOWTD = Week over week to date (last year). Accumulated week to date deviation to corresponding ISO week last year",
                null
            );
...
VAR _Result = DIVIDE(_thisYear-_lastYear,_lastYear)
RETURN _Result", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek),
                ordinal++,
                "WOWTD % = Week over week to date (Last year). % Change for accumulated week to date to corresponding ISO week last year",
                @"""0%"""
            );
...
IF (
    ...
    BLANK ()
)", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek, _ISO_yearWeekRunningNumber),
            ordinal++,
            "Accumulated week to date previous ISO week (using running number)",
            null
        );
...
VAR _Result = _thisYear-_lastWeek
RETURN _Result", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek, _ISO_yearWeekRunningNumber),
                ordinal++,
                "WOPWTD = Week over previous week to date: Accumulated week to date difference to previous ISO week",
                null
            );
...
VAR _Result = DIVIDE(_thisYear-_lastWeek,_lastWeek)
RETURN _Result", _ISO_year, _ISO_week, _calendarTableName, _CalendarDate, _dayNoOfWeek, _ISO_yearWeekRunningNumber),
                ordinal++,
                "WOPWTD % = Week over previous week to date %: Accumulated week to date index: % change from previous ISO week",
                @"""0%"""
            );
        }
        else
        {
             Warning("WTD calculations skipped - missing required ISO year, ISO week, day number, or ISO year week running number columns.");
        }
